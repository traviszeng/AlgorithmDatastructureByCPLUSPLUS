...
template <class T>  istream& operator >> (istream &in, Tree<T> &tree) {
	tree.CreateTree(in, tree.root);
	return in;
}

template <class T>  ostream& operator << (ostream &out, Tree<T> &tree) {
	tree.IntendedText(out);
	return out;
}

template <class T> bool Tree<T>::Root() {		//让树的根结点成为树的当前结点
	if (root == NULL) {
		current = NULL;
		return false;
	}
...
template <class T>bool Tree<T>::Parent() {		//置当前结点的双亲结点为当前结点
...
template <class T>bool Tree<T>::FindParent(TreeNode<T> *t, TreeNode<T> *p) {	//在根为*t的树中找*p的双亲, 并置为当前结点
...
template <class T>bool Tree<T>::FirstChild() {	//在树中找当前结点的长子, 并置为当前结点
...
template <class T> bool Tree<T>::NextSibling() {	//在树中找当前结点的兄弟, 并置为当前结点
...
template <class T> void Tree<T>::PreOrder(TreeNode<T> *subTree, void(*visit)(TreeNode<T> *t)) {
	if (subTree) {
		visit(subTree);
...
template <class T>void Tree<T>::PostOrder(TreeNode<T> *subTree, void(*visit)(TreeNode<T> *t)) {
	if (subTree) {
...
template <class T>void Tree<T>::LevelOrder(TreeNode<T> *subTree, void(*visit)(TreeNode<T> *t)) {	//按广度优先次序分层遍历树, 树的根结点是当前指针current。
...
template <class T>bool Tree<T>::Find(TreeNode <T> *p, T value) {
	bool result = false;
	if (p->data == value) {
		result = true;
		current = p;
...
template<class T>void Tree<T>::RemovesubTree(TreeNode<T> *p) {	//私有函数: 若指针subTree不为空, 则删除根为subTree的子树
	if (subTree) {
...
template <class T>void Tree<T>::IntendedText(TreeNode<T> *subTree, string str, ostream &out) {
	TreeNode<T> *p = subTree;
	while (p) {//深度优先
...
template <class T>void Tree<T>::Output(TreeNode<T> *subTree, string str, ostream &out) {
	if (subTree == root) {
		current = root;
	}
	string temp;
...
template <class T>void Tree<T>::ShowTree(TreeNode<T> *t, ostream &out) {
	if (!t) {
		return;
	}
	out << '(';
...
template <class T>void Tree<T>::CreateTree(istream& in, TreeNode<T> *& subTree) {
	T item;
